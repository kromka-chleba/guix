       ...
       ((which "rustc")
        ...
        (for-each (lambda (wrap-file)
                    (let* ((wrap-name (basename wrap-file ".wrap"))
                      ...
                      (match (find-vendored-input name version-prefix)
                        ((input version)
                         ...
                         ;; Patch source wrap file so that it's configured to
                         ;; "download" from input.  We don't want to just
                         ;; replace the directory, since the download operation
                         ;; may do other things like apply patches.
                  ...
                  (if (file-exists? "subprojects")
                      ...
                      '())))
       ((> (or (and=> (stat vendor-dir) stat:size) 0) 0)
        (format (current-warning-port)
                "warning: rustc is not available; unable to patch wrap files~%"))))))
