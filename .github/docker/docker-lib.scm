...
When WORKSPACE is a non-empty string, bind-mount it at the same absolute path
inside the container and set that path as the working directory.  Mounting at
the same path as the host ensures that a pre-inst-env generated by the host's
./configure (which embeds absolute paths) resolves correctly inside the
container without needing to re-run configure or make inside the container."
