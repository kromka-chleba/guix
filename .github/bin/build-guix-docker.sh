...
echo ""
echo "    To run an interactive shell (Guix images use Shepherd as PID 1,"
echo "    so 'docker run --rm -it ... bash' is blocked; use docker exec instead):"
echo "      CONTAINER=\$(${DOCKER_CMD} run -d --privileged ${FULL_IMAGE_TAG})"
echo "      ${DOCKER_CMD} exec -ti \$CONTAINER /run/current-system/profile/bin/bash --login"
echo "      ${DOCKER_CMD} stop \$CONTAINER"
echo ""
