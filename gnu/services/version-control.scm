...
for more information.")
  ...
  (ckout-alias maybe-string
               "The NAME for /doc/NAME/... to be treated as /doc/ckout/...")
...
(define (fossil-shepherd-service config)
  (match-record config <fossil-configuration>
                (package user group log-file repository acme? base-url
                 ...
                 files from https? ip jail? js-mode local-auth? main-menu
                 max-latency port redirect-to-https? repo-list? skin
                 socket-file socket-mode th-trace? tls-cert tls-private-key)
    (shepherd-service
     (provision '(fossil))
     (requirement '(user-processes networking))
     (start #~(make-forkexec-constructor
               ...
               #:log-file #$log-file))
     (stop #~(make-kill-destructor))
     (documentation "Run the Fossil SCM's HTTP server."))))
...
(define fossil-service-type
  (service-type
    ...
    (description "Run the Fossil SCM's HTTP server.")))
