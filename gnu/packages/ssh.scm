  ...
  (package
    ...
    (source
     (origin
       (method git-fetch)
       (uri (git-reference
             (url "https://codeberg.org/vaporup/ssh-tools.git")
          ...
          (add-after 'install-bash 'install-manpages
            (lambda* (#:key unpack-path #:allow-other-keys)
              (let ((files (find-files (string-append "src/" unpack-path
                ...
                (for-each (cut install-file <> man) files)))))
    ...
    (propagated-inputs (list go-golang-org-x-crypto go-sigs-k8s-io-yaml))
    (home-page "https://codeberg.org/vaporup/ssh-tools")
    (synopsis "Making SSH more convenient")
    (description
     "This package provides a suite of tools to improve the SSH experience.
