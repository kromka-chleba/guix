...
(define-public supercollider-3.13.0
  (package
    (name "supercollider")
    (version "3.13.0")
    (source
     (origin
       ...
       (file-name (git-file-name name version))
       (sha256
        (base32 "1dkpnaly4m2j41ypy7xj5m2yhbl4ykw3vbnam345z4dk6qhyj9b1"))
       (patches (search-patches "supercollider-env-data.diff"))
       (modules '((guix build utils)
                  (ice-9 ftw)))
       (snippet
        ;; The build system doesn't allow us to unbundle the following
     ...
     (list
      ...
      #~(modify-phases %standard-phases
          ...
          (add-after 'install 'install-ide
            (lambda _args
              (map (lambda (moved)
                     (mkdir-p (string-append #$output:ide
                                             (car moved)))
                     (rename-file (apply string-append
                                         #$output moved)
                                  (apply string-append
                                         #$output:ide moved)))
                   '(("/bin" "/scide")
                     ("/share/applications" "/SuperColliderIDE.desktop"))))))))
