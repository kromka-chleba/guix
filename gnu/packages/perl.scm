...
(define-public perl-io-pty
  (package
    ...
    (description
     "@code{IO::Pty} provides an interface to allow the creation of a pseudo
tty.  @code{IO::Pty} inherits from @code{IO::Handle} and so provide all the
methods defined by the @code{IO::Handle} package.")
