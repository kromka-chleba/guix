      ...
      #~(modify-phases %standard-phases
          ...
          (add-after 'generate-injected-scripts 'add-build-script
            (lambda _
              ...
              ;; Compile TypeScript and copy non-TS files (JS, JSON).
              (modify-json
               (lambda (pkg)
                 (acons "scripts"
                        '(("build" . "find src -name '*.ts' | xargs esbuild --outdir=lib --format=cjs --platform=node && cp -r src/third_party lib/ && find src -name '*.json' -exec sh -c 'mkdir -p lib/$(dirname ${0#src/}) && cp $0 lib/${0#src/}' {} \\;"))
                        pkg)))))
          ...
          (add-after 'install 'wrap-playwright-cli
            (lambda* (#:key inputs outputs #:allow-other-keys)
              (let* ((out (assoc-ref outputs "out"))
                     (chromium (search-input-file inputs "/bin/chromium"))
...
@strong{Important}: This Guix package only supports Chromium-based browsers.
