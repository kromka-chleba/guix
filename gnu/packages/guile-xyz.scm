...
@code{init_socket}.
                Probably can be replaced with guile's built-ins: @code{fdes->ports}, @code{recv!}, @code{send}")
    ...
    (arguments
     (list
      ...
      #~(list
         "GUILE_AUTO_COMPILE=0"
         (string-append "CC=" #$(cc-for-target))
         (string-append "CFLAGS=" "-I" #$(this-package-input "guile") "/include/guile/3.0")
         (string-append "PREFIX=" #$output))
      #:phases
      #~(modify-phases %standard-phases
          (delete 'configure)
          (add-after 'unpack 'chdir
            (lambda _ (chdir "ext/shm") #t))
          (add-before 'build 'respect-guile
            (lambda* (#:key outputs #:allow-other-keys)
              (delete-file "Makefile")
              (rename-file "guile.Makefile" "Makefile")))
          ...
          (add-after 'build-guile 'install-guile
            (lambda* (#:key outputs #:allow-other-keys)
              (let* [(v (target-guile-effective-version))
                     (share-dir (string-append #$output "/share/guile/site/" v "/wayland-scm"))]
...
(define-public guile-wayland-scm
  (package
    ...
    (synopsis "Mostly R7RS-compliant Guile Scheme client bingings for Wayland")
    (description "")
    (home-page "https://codeberg.org/shegeley/wayland-scm")
    (license license:wtfpl2)))
