...
(define* (make-ldc version hash
                   ...
                   (pkg-bootstrap-dmd (delay gdmd-11))
                   (pkg-llvm (delay llvm))
                   (pkg-clang-runtime (delay clang-runtime))
                   (pkg-clang (delay clang)))
  ...
  (define (dmd-test-path subpath)
    (string-append
     (if (until-version? "1.31")
         ...
         "tests/dmd")
     subpath))
  (define* (make-ldc-stage stage pkg-prev-stage)
    (package
      (name name)
      (version version)
      (source
      ...
      (inputs
       (list (force* pkg-llvm)
             (force* pkg-clang-runtime)
             libconfig
             zlib
             (force* pkg-clang)         ; Used as a linker wrapper.
             tzdata                     ; std.datetime.timezone
             curl                       ; std.net.curl
             bash-minimal))             ; std.process
      (native-inputs
       (list (force* (cond ((= stage 1) pkg-bootstrap-dmd)
                           ((= stage 2) pkg-prev-stage)))
             (force* pkg-clang)
             lld-as-ld-wrapper
             ;; For testing
      ...
      (arguments
       (list
        ...
        (cond ((= stage 1) (list))
              ((= stage 2) (list (force* pkg-bootstrap-dmd))))
        #:generator "Ninja"
        #:configure-flags
        (if (target-riscv64?)
          ...
          #~(modify-phases %standard-phases
              ...
              (add-after 'unpack 'patch-tests
                (lambda _
                  ...
                  ;; These are tests I wrote for dmd, but one hangs when a
                  ;; modern llvm opt is applied. Fix by only running debug
                  ;; builds.
...
with modern optimization and code generation capabilities. The compiler uses the
official DMD frontend to support the latest version of D2, and relies on the
...
(define-public ldc-1.29
  (make-ldc "1.29.0" "0q8lhap4j13z0g6mhc404mcr12dqkiwc1hl13iw70zls5bdfy78j"
            ...
            #:pkg-bootstrap-dmd ldc-1.25
            #:pkg-llvm (delay llvm-14)
            #:pkg-clang-runtime (delay clang-runtime-14)))
...
(define-public ldc-1.32
  (make-ldc "1.32.2" "0h5h960ydcx2i786i8z8wyw5i8mwvwmnr0j6q8fy8cmpmi8hw3rd"
            ...
            #:pkg-bootstrap-dmd ldc-1.25
            #:pkg-llvm (delay llvm-15)
            #:pkg-clang-runtime (delay clang-runtime-15)))
...
(define-public ldc-1.34
  (make-ldc "1.34.0" "1hg0053dm06ndf4mapiihv0vz1p3l1q2ywc3zbxh9x0xz8axi9v3"
            ...
            #:pkg-bootstrap-dmd ldc-1.25
            #:pkg-llvm (delay llvm-16)
            #:pkg-clang-runtime (delay clang-runtime-16)))
...
(define-public ldc-1.36
  (make-ldc "1.36.0" "1z22zhy10j1gy77zrszk0rp48wf86mvrz12jpvass8mx02z3wmxl"
            ...
            #:pkg-bootstrap-dmd ldc-1.25
            #:pkg-llvm (delay llvm-17)
            #:pkg-clang-runtime (delay clang-runtime-17)))
...
(define-public ldc-1.38
  (make-ldc "1.38.0" "068gqv368mhi9jywk9dcx9xssywcix5ypixxs9hi87cz3w913xbp"
            ...
            #:pkg-bootstrap-dmd ldc-1.25
            #:pkg-llvm (delay llvm-18)
            #:pkg-clang-runtime (delay clang-runtime-18)))
(define-public ldc-1.39
  (make-ldc "1.39.0" "0spa8170sm4lskjq2qja1ciymyz16j7dvb3vv8p0ps0q7c0v88b6"
            ...
            #:pkg-llvm (delay llvm-18)
            #:pkg-clang-runtime (delay clang-runtime-18)))
(define-public ldc-1.40
  (make-ldc "1.40.1" "1sdsxryr9i5j7xp17mlj7w0xl6c7nrhvjdyn6h69l0xrgmsg1nar"
            #:frontend-version "2.110.0"
            #:pkg-bootstrap-dmd ldc-1.25
            #:pkg-llvm (delay llvm-19)
            #:pkg-clang-runtime (delay clang-runtime-19)))
(define-public ldc-1.41
  (make-ldc "1.41.0" "1sxkfy8i414csg4fpzzybbjnxqrcgxdar1j8h05aw56jf9ijkdz8"
            #:frontend-version "2.111.0"
            #:pkg-bootstrap-dmd ldc-1.25
            #:pkg-llvm (delay llvm-20)
            #:pkg-clang-runtime (delay clang-runtime-20)))
  ...
  (package
    ...
    (source
     (origin
       ...
       (file-name (git-file-name name version))
       (sha256
        (base32 "1ism6wziki5lzsnwq2p144pwgjk1rqj0j85xnciaiswz8a2dqais"))))
