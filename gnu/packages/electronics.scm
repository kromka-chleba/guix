  ...
  (package
    ...
    (source
     (origin
       ...
       (sha256
        (base32 "0p0nmm7mgsibpys9zvcxxmw3h3a61fhyadnj5s6glwgghlzsc216"))))
    (build-system pyproject-build-system)
          ...
          (add-after 'install 'run-icestorm-examples
            (lambda* (#:key tests? #:allow-other-keys)
              (when tests?
                ...
                (copy-recursively
                 (string-append
                  #$(this-package-input "icestorm") "/examples")
          ...
          (add-after 'run-icestorm-examples 'run-prjtrellis-examples
            (lambda* (#:key tests? #:allow-other-keys)
              (when tests?
                ...
                (copy-recursively
                 (string-append
                  #$(this-package-input "prjtrellis") "/examples")
     ...
     (list
      ...
      #~(modify-phases %standard-phases
          (add-after 'unpack 'chdir
            (lambda _
              (chdir "libgm")))
          (add-after 'install 'keep-delay
            (lambda _
              (copy-recursively
               "../delay" (string-append #$output "/delay")))))))
    (inputs
     (list boost))
    (synopsis "GateMate FPGAs bitstream tools and database")
    (description
     "@code{Prjpeppercorn} includes programming tools data needed to create
@code{nextpnr} chip database for GateMate architecture from Cologne Chip.")
