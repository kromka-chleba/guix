...
require to know the structure of the payload (eg.  create structs), and
allows accessing fields by providing the path to them.")
    (license license:expat)))
...
(define-public go-github-com-hashicorp-serf
  (package
     ...
     (list
      ...
      #~(list "-skip"
              (string-join
               ...
               ;; [ERR] agent: Error invoking script ...
    ...
    (propagated-inputs (list go-github-com-armon-circbuf
                             go-github-com-hashicorp-go-metrics
    ...
    (synopsis "Service discovery and orchestration for Go")
    (description
...
client/server library in Go. @code{mDNS} can be used to discover services
on the local network without the use of an authoritative DNS server.
...
(define-public go-sigs-k8s-io-knftables
  (package
    ...
    (description
     "This package provides nftables bindings in Go. It is not
intended to support arbitrary use cases, but instead specifically focuses
on supporting Kubernetes components which are using nftables.")
    (license license:asl2.0)))
